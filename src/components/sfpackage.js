...
const ListItem = ({ pkg }) => {
  ...
  return (
    <div>
      ...
      {startingfrom && (
        <TrekVitals heading={'Trek Starts From'} value={startingfrom} />
      )}
      {endsat && <TrekVitals heading={'Trek Ends At'} value={endsat} />}
      {packagetype && <TrekVitals heading={'Trek Type'} value={packagetype} />}
      {noofdays && (
        <TrekVitals heading={'Total Days'} value={noofdays + ' days'} />
      )}
      {minimumage && (
        <TrekVitals heading={'Minimum Age'} value={minimumage + ' years'} />
      )}
      {trekkingdays && (
        <TrekVitals heading={'Trekking days'} value={trekkingdays + ' days'} />
      )}
      {trekkingdistance && (
        <TrekVitals
          heading={'Trekking Distance'}
          value={trekkingdistance + ' km'}
        />
      )}
      {transportation && (
        <TrekVitals heading={'Transportation'} value={transportation + ' km'} />
      )}
      {fooddetails && <TrekVitals heading={'Food'} value={fooddetails} />}
      {accommodation && (
        <TrekVitals heading={'Accomodation'} value={accommodation} />
      )}
    </div>
  );
};
  ...
  return (
    ...
    >
      ...
      {inclusions && inclusions.length > 0 && (
        <Inclusions items={inclusions} title={'Package Inclusions'} />
      )}
      <br />
      {exclusions && exclusions.length > 0 && (
        <Inclusions items={exclusions} title={'Package Exclusions'} />
      )}
      <br />
...
const SfPackageData = ({ packages }) => {
  return packages.map((pkg) => (
    <TrekPackage key={pkg.node.packagename} pkg={pkg.node} />
  ));
};
